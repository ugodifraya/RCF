...
export default function Profile() {
  const { user, setUser, isCoach } = useAuth();
  ...
  const savePassword = async () => {
    ...
    if (newPassword.length < 6) {
      setPwdMsg({ type: 'err', text: 'Au moins 6 caractères requis.' });
      return;
    }
    setPwdSaving(true);
    setPwdMsg(null);
  ...
  const saveMeasurement = async () => {
    if (!weight && !height) { setMeasMsg({ type: 'err', text: 'Entrez au moins le poids ou la taille.' }); return; }
    setMeasSaving(true); setMeasMsg(null);
    try {
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="card flex items-center gap-5">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">{isCoach ? 'Coach' : 'Joueuse'}{user?.position ? ` · ${user.position}` : ''}</p>
      ...
      {/* Rappel mensuel (joueuse uniquement) */}
      {!isCoach && showReminder && (
        <div className="card bg-amber-50 border-amber-200">
          <p className="text-sm font-semibold text-amber-800">Rappel mensuel</p>
          <p className="text-sm text-amber-700 mt-0.5">Pensez à mettre à jour votre poids et votre taille.</p>
        </div>
      )}

      {/* Mesures physiques (joueuse uniquement) */}
      {!isCoach && (
        <div className="card space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div><label className="label">Poids (kg)</label><input type="number" step="0.1" className="input" placeholder="Ex: 62.5" value={weight} onChange={e => setWeight(e.target.value)} /></div>
            <div><label className="label">Taille (cm)</label><input type="number" step="0.1" className="input" placeholder="Ex: 168" value={height} onChange={e => setHeight(e.target.value)} /></div>
            <div><label className="label">Date</label><input type="date" className="input" value={measDate} onChange={e => setMeasDate(e.target.value)} /></div>
          </div>
          {measMsg && <p className={`text-sm ${measMsg.type === 'ok' ? 'text-green-600' : 'text-red-600'}`}>{measMsg.text}</p>}
      ...
      <div className="card space-y-4">
        ...
        <div><label className="label">Mot de passe actuel</label><input type="password" className="input" value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} /></div>
        <div><label className="label">Nouveau mot de passe</label><input type="password" className="input" value={newPassword} onChange={e => setNewPassword(e.target.value)} /></div>
        <div><label className="label">Confirmer</label><input type="password" className="input" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} /></div>
